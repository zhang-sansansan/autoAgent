...
public class AutoAgentTest {
    ...
    @Before//被该注解标记的方法会在这个类的每个方法执行前执行一次
    public void init() {
        //初始化Api
        ...
        OpenAiChatModel chatModel = OpenAiChatModel.builder()
                ...
                .defaultOptions(OpenAiChatOptions.builder()
                        ...
                        .build())
                .build();
        // 初始化agent客户端 Planning Agent ChatClient - 负责任务规划
    ...
    public void test_planning_agent(){
        ...
        String content = planningChatClient
                .prompt(userRequest)//给客户端输入用户请求
                .system(s -> s.param("current_date", LocalDate.now().toString()))//填充人设的占位符
                .advisors(a -> a
                        .param(CHAT_MEMORY_CONVERSATION_ID_KEY, "planning-session-001")//记录本次会话id，便于记忆回溯
                        .param(CHAT_MEMORY_RETRIEVE_SIZE_KEY, 50))//设置记忆回溯的条数
                .call().content();

        log.info("规划结果: {}", content);
    ...
    /**
     * 测试 React Agent - 响应式处理功能  对应第一种
     */
    //调用多个agent，每个agent根据输入的命令返回结果，链式，就是将上一个流程的agent的返回结果的String作为当前agent的命令入参一部分
    @Test
