...
export const AdvisorManagement: React.FC = () => {
  ...
  const handleUpdateAdvisor = async () => {
    ...
    try { JSON.parse(editFormData.extParam.trim() || '{}'); } catch { Toast.error('扩展参数必须是有效的 JSON'); return; }
  ...
  const handleCreateAdvisor = async () => {
    ...
    try { JSON.parse(createFormData.extParam.trim() || '{}'); } catch { Toast.error('扩展参数必须是有效的 JSON'); return; }
