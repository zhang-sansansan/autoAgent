...
export const AiClientApiManagement: React.FC = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'apiKey',
      width: 200,
      render: (text: string) => <Space><span>{text ? `••••••••${text.slice(-4)}` : '—'}</span>{text && <Button aria-label="复制 API Key" theme="borderless" size="small" icon={<IconCopy />} onClick={() => handleCopyApiKey(text)} />}</Space>,
    },
