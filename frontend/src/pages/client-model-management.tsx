...
import { aiClientApiAdminService, AiClientApiResponseDTO } from '../services/ai-client-api-admin-service';
import { 
...
export const ClientModelManagement: React.FC = () => {
  ...
  // 侧边栏状态
  const [sidebarCollapsed, setSidebarCollapsed] = useState(() => localStorage.getItem('ann-agent-studio:sidebar-collapsed') === 'true');
  
  // 表格数据状态
  const [modelList, setModelList] = useState<AiClientModelResponseDTO[]>([]);
  const [apiOptions, setApiOptions] = useState<AiClientApiResponseDTO[]>([]);
  ...
  const handleSave = async () => {
    try {
      ...
      if (formData.apiId && !apiOptions.some((api) => api.apiId === formData.apiId)) {
        Toast.error('关联的模型 API 不存在，请先创建 API 配置');
        return;
      }
  ...
  useEffect(() => {
    aiClientApiAdminService.queryAllAiClientApis().then((result) => {
      if (result.code === '0000') setApiOptions(result.data || []);
    }).catch(() => Toast.error('模型 API 选项加载失败'));
  }, []);
      ...
      >
        ...
        <FormRow>
          ...
          <Select
            ...
            onChange={(value) => setFormData({ ...formData, apiId: String(value ?? '') })}
          >
            {apiOptions.map((api) => <Select.Option key={api.apiId} value={api.apiId}>{api.apiId}</Select.Option>)}
          </Select>
        </FormRow>
